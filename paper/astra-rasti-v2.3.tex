% astra-rasti.tex — ASTRA RASTI Paper V2.2
...
%   - Added 8 bibliography entries: Okun1962, Phillips1958, Preston1975,
%     ReinhartRogoff2010, BenjaminiHochberg1995, NOAA2024, Hansen2010, WorldBank2024
...
% ======================================================================
\title[ASTRA: An Integrated Analysis Framework]{ASTRA: An Integrated Analysis Framework for Physics-Aware Astrophysical Discovery}

\author[G.\,J.~White \& R.~Dey]{%
...
$^{3}$VBRL Holdings Inc, 1309 Coffeen Avenue STE~1200, Sheridan, WY\,82801, USA}
...
We validate the framework through six controlled astrophysical test cases drawn from nine independent data sources spanning more than 27\,430 data points---Herschel filament scaling relations, Chandra Deep Field South multi-wavelength cross-matching, SDSS galaxy property analysis, Gaia stellar causal inference, Bayesian model comparison, and a discovery-mode demonstration on synthetic data with known ground truth.
...
To test cross-domain generalization, we further deploy \astra on public data from economics (World Bank; \citealt{WorldBank2024}), climate science (NASA GISS; NOAA Mauna Loa; \citealt{Hansen2010,NOAA2024}), and epidemiology (World Bank Health indicators), recovering approximately 40 validated hypotheses across five scientific domains---including well-established relationships such as Okun's Law, the Preston Curve, and CO$_{2}$--temperature coupling---while correctly identifying non-relationships (e.g., the Phillips Curve) and applying false discovery rate correction \citep{BenjaminiHochberg1995} to cross-domain syntheses.
We discuss capabilities and limitations, emphasising \astra's role as a tool to assist scientists rather than replace domain expertise.
...
Section~\ref{sec:discussion} discusses why these results demonstrate \astra's unique capabilities, clarifies \astra's role as a tool to assist rather than replace scientists, and Section~\ref{sec:conclusion} concludes.
...
The six astrophysical test cases in this paper show \astra can: discover physical laws from data with theoretical validation; fuse multi-wavelength observations with proper uncertainty handling; identify established astrophysical relationships through pattern recognition; distinguish causation from correlation; perform rigorous Bayesian model comparison; and operate in knowledge isolation mode to discover patterns and generate testable predictions beyond knowledge retrieval.
The cross-domain validation (Section~\ref{sec:crossdomain}) further demonstrates that these analytical capabilities---dimensional analysis, causal reasoning, hypothesis testing---generalize to economics, climate science, and epidemiology, recovering approximately 40 validated hypotheses across five scientific domains.
...
The cross-domain validation (Section~\ref{sec:crossdomain}) extends the validation scope to five scientific domains---astrophysics, economics, climate science, epidemiology, and cross-domain synthesis---using exclusively public data from established archives.
...
\begin{itemize}
  ...
  \item 0 Normal galaxies classified (reflecting the bias toward X-ray-bright AGN in the CDFS rather than the absence of galaxies in the field; normal galaxies are present in the optical catalogue but are below the X-ray detection threshold and hence not cross-matched)
\end{itemize}
...
\end{itemize}
These priors are chosen to be broad enough to accommodate all physically reasonable values while providing proper normalization for the Bayesian evidence integral.
...
We constructed a synthetic dataset of 500 molecular cloud regions with the following causal structure (known to the authors, hidden from \astra):
...
This test case demonstrates \astra's capacity for automated pattern discovery and triage in survey-scale datasets, complementing the hypothesis-driven validation of Test Cases~1--5.
...
A critical test for any general-purpose analytical framework is whether its capabilities transfer beyond the domain for which it was originally developed.
...
In each case, \astra operated without domain-specific guidance: the system ingested the data, autonomously identified patterns, applied its causal and statistical reasoning modules, and generated hypotheses that we compare with established results from the relevant literatures.
...
\begin{enumerate}
  \item[(i)] \textbf{Okun's Law:} \astra recovers a negative relationship between GDP growth and unemployment change, consistent with the empirical regularity reported by \citet{Okun1962}. The estimated coefficient ($-1.8$\,per\,cent unemployment per 1\,per\,cent GDP shortfall) is within the range reported in the literature.
  ...
  \item[(iii)] \textbf{Gini inequality persistence:} Income inequality, as measured by the Gini coefficient, exhibits strong temporal persistence ($r = 0.92$ at 5-year lag), consistent with structural theories of inequality.
...
\begin{enumerate}
  \item[(x)] \textbf{Phillips Curve:} \astra tested for a negative relationship between unemployment and inflation, as originally reported by \citet{Phillips1958}. The system found no statistically significant relationship in the post-1990 cross-country data ($r = -0.03$, $p = 0.62$). This negative result is consistent with the well-documented breakdown of the simple Phillips Curve in modern macroeconomic data and demonstrates that \astra correctly identifies non-relationships rather than forcing spurious patterns.
\end{enumerate}
...
\begin{enumerate}
  \item[(i)] \textbf{Infant mortality vs.\ GDP:} \astra identifies a strong negative relationship between infant mortality rate and GDP per capita ($R^{2} = 0.587$), with a logarithmic functional form providing the best fit---consistent with the diminishing marginal returns of wealth on health outcomes.
...
The application of FDR correction is methodologically important: without multiple testing correction, the cross-domain analysis would have reported 8/8 validated hypotheses, a misleadingly optimistic result. \astra's automatic application of FDR correction demonstrates responsible statistical practice.
...
\textbf{Phillips Curve failure:} As discussed in Section~\ref{sec:crossdomain:economics}, \astra found no statistically significant unemployment--inflation trade-off in post-1990 data ($r = -0.03$, $p = 0.62$). This is consistent with the well-documented instability of the Phillips Curve and demonstrates that \astra does not force patterns where none exist.
...
\textbf{Confound identification:} In 3 of the 5 validated cross-domain hypotheses, \astra's causal module identified confounding variables (primarily GDP as a common cause) and flagged the correlations as potentially non-causal. This is a critical capability: reporting a correlation between CO$_{2}$ emissions and life expectancy without noting the GDP confound would be scientifically irresponsible.
...
In total, \astra tested approximately 42 hypotheses across five scientific domains, validating 36 (including 1 with caveats), failing 1 (Phillips Curve), finding 1 inconclusive (ENSO), and losing 3 after FDR correction.
This 86\,per\,cent validation rate---achieved without domain-specific tuning and across disciplines as diverse as astrophysics, macroeconomics, and epidemiology---provides strong evidence that \astra's analytical capabilities generalize beyond its original astrophysical domain.
...
The six controlled astrophysical test cases, five live deployments, and cross-domain validation in this paper collectively analyse data drawn from more than a dozen independent sources spanning five scientific domains.
The astrophysical analyses process more than 27\,430 data points from nine sources (Herschel, Chandra, SDSS, Gaia, NASA Exoplanet Archive, Pantheon+, synthetic molecular clouds, MPA-JHU value-added catalogue, and Gaia EDR3), while the cross-domain validation draws on national-level panel data for 217 countries over six decades (World Bank), global temperature records spanning 143 years (NASA GISS), and atmospheric CO$_{2}$ measurements covering 65 years (NOAA Mauna Loa).
...
\begin{enumerate}
  ...
  \item[(iv)] \textbf{Cross-domain generalization} (Section~\ref{sec:crossdomain}): \astra operates on non-astrophysical data from economics, climate science, and epidemiology, recovering established relationships and correctly identifying non-relationships---demonstrating that the framework's analytical capabilities are not domain-specific artefacts.
\end{enumerate}
...
For the cross-domain validation, the value lies in demonstrating that \astra's analytical architecture generalizes beyond astrophysics.
...
The cross-domain results (Section~\ref{sec:crossdomain}) extend this argument: recovering Okun's Law, the Preston Curve, and CO$_{2}$--temperature coupling from raw public data---using the same analytical pipeline that recovers Kepler's law and galaxy bimodality---demonstrates that \astra's capabilities are genuinely domain-general rather than astrophysics-specific.
The Phillips Curve failure and FDR attrition of cross-domain hypotheses provide further evidence that the system produces honest, calibrated results rather than indiscriminate confirmations.
...
Fourth, the \emph{cross-domain generalization}: the same analytical pipeline that recovers astrophysical scaling relations also recovers macroeconomic regularities, climate trends, and epidemiological relationships, with appropriate epistemic qualifications including confound identification, FDR correction, and negative result reporting.

\subsection{Cross-Domain Generalization}
\label{sec:discussion:crossdomain}

The cross-domain validation (Section~\ref{sec:crossdomain}) addresses a fundamental question about \astra's design: is the framework genuinely general-purpose, or are its capabilities specific to the astrophysical domain for which it was developed?

Three aspects of the cross-domain results support generality.
First, the \emph{same analytical modules}---dimensional analysis, causal inference, Bayesian model selection, pattern recognition---that recover astrophysical relationships also recover economic, climatic, and epidemiological relationships, without any domain-specific modifications.
Second, \astra's \emph{epistemic qualifications} transfer across domains: the system correctly identifies GDP as a confound in the life expectancy--CO$_{2}$ correlation, flags the Reinhart--Rogoff threshold as sensitive to outliers and endogeneity, and applies FDR correction to the cross-domain synthesis---demonstrating that its causal reasoning and statistical rigour are not limited to astrophysical contexts.
Third, the \emph{negative results}---the Phillips Curve failure, the ENSO inconclusive result, and the three FDR-lost cross-domain hypotheses---demonstrate that \astra does not indiscriminately confirm hypotheses, even in domains where the system has no encoded knowledge to guide it toward ``correct'' answers.

These results suggest that \astra's architecture is genuinely domain-agnostic at the analytical level, even though the system was originally developed for astrophysics.
The core capabilities---pattern discovery, causal inference, hypothesis testing with multiple testing correction, confound identification---are domain-general by construction, since they rely on statistical and causal methods that are not specific to any scientific field.

\textbf{Caveats:}
The cross-domain validation uses well-established relationships as benchmarks. \astra has not been tested on genuinely open questions in economics, climate science, or epidemiology, and its performance on such problems remains unknown.
The domain knowledge systems (MORK ontology, 75 specialized modules) are astrophysics-specific; extending \astra to production-quality analysis in other domains would require developing analogous domain knowledge for those fields.
The cross-domain results should be understood as evidence that the \emph{analytical architecture} generalizes, not that \astra is ready for deployment as an economics or climate science tool.
...
\begin{itemize}
  ...
  \item \textbf{Cross-domain depth:} The cross-domain validation uses well-established benchmarks and has not been tested on open questions in non-astrophysical domains. Production-quality analysis in economics, climate science, or epidemiology would require domain-specific knowledge modules analogous to \astra's astrophysical MORK ontology.
\end{itemize}
...
We have presented \astra (Autonomous System for Scientific Discovery in Astrophysics), an integrated framework for physics-aware scientific analysis.
Through six controlled astrophysical test cases, five live deployments on archival data, and cross-domain validation across economics, climate science, and epidemiology, we have demonstrated \astra's technical capabilities across approximately 42 hypotheses in five scientific domains.
...
\textbf{Cross-Domain Validation} (Section~\ref{sec:crossdomain})
\begin{itemize}
  \item Economics: 9/10 hypotheses validated (Okun's Law, trade--GDP, inequality persistence, and 6 others); Phillips Curve correctly identified as non-significant
  \item Climate science: 4/5 hypotheses validated (CO$_{2}$--temperature $R^{2}=0.936$, warming acceleration, CO$_{2}$ growth, decadal trend)
  \item Epidemiology: 5/5 hypotheses validated (Preston Curve $R^{2}=0.686$, infant mortality, vaccination, maternal mortality, health spending)
  \item Cross-domain synthesis: 5/8 hypotheses survived FDR correction (Wealth--Health Nexus $R^{2}=0.700$ the strongest); 3 lost after multiple testing correction
\end{itemize}
...
The cross-domain validation demonstrates that \astra's analytical capabilities---pattern discovery, causal inference, hypothesis testing, confound identification---generalize beyond astrophysics to economics, climate science, and epidemiology (\emph{cross-domain generalization mode}).
...
The cross-domain results suggest that this integration is genuinely domain-general, opening the possibility of applying \astra's analytical architecture to scientific problems beyond astrophysics.
...
The development of ASTRA follows from work on stigmergy and autonomous intelligence by \citet{Dey2025} at OpenHub, Thailand.
...
RD developed the core codebase, implemented the integration architecture, and contributed to testing.
...
The scientific content, analysis, and conclusions were designed, executed, and verified by the authors.
